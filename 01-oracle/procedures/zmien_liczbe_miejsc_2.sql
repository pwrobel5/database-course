...
BEGIN
    ...
    WHERE wp.ID_WYCIECZKI = ZMIEN_LICZBE_MIEJSC_2.id_wycieczki;
    ...
    FROM WYCIECZKI w
    WHERE w.ID_WYCIECZKI = ZMIEN_LICZBE_MIEJSC_2.id_wycieczki;

    IF ZMIEN_LICZBE_MIEJSC_2.liczba_miejsc < liczba_zajetych_miejsc THEN
        raise_application_error(-20108, 'Nowa liczba miejsc jest mniejsza 
        niż liczba miejsc zajętych!');
    END IF;

    UPDATE WYCIECZKI
    SET LICZBA_MIEJSC = ZMIEN_LICZBE_MIEJSC_2.liczba_miejsc,
        LICZBA_WOLNYCH_MIEJSC = 
            ZMIEN_LICZBE_MIEJSC_2.liczba_miejsc - liczba_zajetych_miejsc
